...
async def check_card_site(
    ...
    for attempt in range(_MAX_RETRIES + 1):
        # rotate proxy على كل retry لو في list
        if attempt > 0 and isinstance(proxy_data, list) and len(proxy_data) > 1:
            proxy_str = _proxy_data_to_proxy_str(random.choice(proxy_data))
            url = f"{API_BASE}/check?card={cc_str}&url={site_url}&proxy={proxy_str}"
