...
TOKEN = "8962918744:AAHorUmwpsl09wUkqx8_Vn5nK9jkWXnk8DM"

join_channel_id = -1004329229984
join_chat_id = -1004349648366

CHANNEL_LINK = "http://t.me/onyxa_cc"
GROUP_LINK = "http://t.me/onyxa_chat"

POWERED_BY = "Powered by @iizxy"
BOT_NAME = "𝗵𝗰𝗵𝗸.𝗰𝗮𝗿𝗱𝘀"
...
E = {
    ...
    # مفاتيح ناقصة — مضافة بعد الإصلاح
    "next": "5084974483685507801",
}
...
async def global_error_handler(event: types.ErrorEvent):
    ...
    if isinstance(exc, TelegramForbiddenError):
        return True
    # معالجة conflict — نسختين شغالين في نفس الوقت
    from aiogram.exceptions import TelegramConflictError
    if isinstance(exc, TelegramConflictError):
        log.critical("CONFLICT: نسختان من البوت شغالتان! يجب إيقاف إحداهما.")
...
async def main():
    ...
    # حذف أي webhook قديم + إيقاف أي جلسة polling سابقة
    try:
        await bot.delete_webhook(drop_pending_updates=True)
        log.info("تم حذف الـ webhook وتجاهل التحديثات المعلقة")
    except Exception as exc:
        log.warning("فشل حذف الـ webhook: %s", exc)

    # انتظار صغير لضمان إنهاء أي instance ثاني
...
if __name__ == "__main__":
    ...
    # قفل ملف لمنع تشغيل نسختين في نفس الوقت
    _LOCK_FILE = "/tmp/onyxa_bot.lock"
    try:
        ...
        print("❌ البوت شغال بالفعل! أوقف النسخة الأخرى أولاً.")
        sys.exit(1)
